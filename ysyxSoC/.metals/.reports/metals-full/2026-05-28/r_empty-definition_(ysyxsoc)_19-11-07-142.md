error id: file://<WORKSPACE>/src/SoC.scala:AddressSet
file://<WORKSPACE>/src/SoC.scala
empty definition using pc, found symbol in pc: 
semanticdb not found

found definition using fallback; symbol AddressSet
offset: 2045
...
class ysyxSoCASIC(implicit p: Parameters) extends LazyModule {
  ...
  val lclint = LazyModule(new AXI4CLINT(AddressSet.misaligned(0x02000000L, 0x10000)))
  val lmrom = LazyModule(new AXI4MROM(AddressSet.misaligned(0x20000000, 0x1000)))
  val sramNode = AXI4RAM(Ad@@dressSet.misaligned(0x0f000000, 0x2000).head, false, true, 4, None, Nil, false)
  ...
  List(apbxbar := APBDelayer() := AXI4ToAPB() := AXI4Buffer(), lclint.node, lmrom.node, sramNode).map(_ := xbar2)
