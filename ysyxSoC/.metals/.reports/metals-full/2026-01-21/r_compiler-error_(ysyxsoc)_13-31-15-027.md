error id: 6453DCA93A17FACCA60F57669B379DE6
...
class APBSPI(address: Seq[AddressSet])(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    xipThread.entry {
      ...
      // Step 1: 执行具体的 XIP 逻辑
      // 注意：这里传入的是寄存器锁存的地址，防止总线抖动
      val addrReg = RegEnable(in.paddr, flashHit && in.penable)
      val retData = xipThread.Call(xipFunc, addrReg)

      // Step 2: 输出结果并等待总线释放 (Done 状态)
      xipThread.Step {
        in.pready := true.B
        in.prdata := retData
        in.pslverr := false.B

        // 关键：保持在当前 Step，直到 CPU 撤销 psel
        // 这样能确保 pready 信号足够稳定
        xipThread.pc := xipThread.pc 
        
        when (!in.psel) {
          // 只有当总线完全释放后，才回到 Step 0 等待下一次
          xipThread.pc := 0.U 
        }
      }
      ...
      // Global 监控：依然有效
      xipThread.Global {
        when(xipThread.pc =/= 0.U) {
          printf(p"XIP_THREAD Running: PC=${xipThread.pc}, PREADY=${in.pready}\n")
        }
      }
    }
