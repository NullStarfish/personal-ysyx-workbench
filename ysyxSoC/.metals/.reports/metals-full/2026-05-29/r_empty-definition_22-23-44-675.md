error id: file://<WORKSPACE>/src/device/SDRAM.scala:
...
non-local guesses:
	 ...
	 -chisel3/halfAddr#
	 -chisel3/halfAddr().
	 -chisel3/util/halfAddr.
	 -chisel3/util/halfAddr#
	 -chisel3/util/halfAddr().
	 -freechips/rocketchip/amba/axi4/halfAddr.
	 -freechips/rocketchip/amba/axi4/halfAddr#
	 -freechips/rocketchip/amba/axi4/halfAddr().
	 -freechips/rocketchip/amba/apb/halfAddr.
	 -freechips/rocketchip/amba/apb/halfAddr#
	 -freechips/rocketchip/amba/apb/halfAddr().
	 -freechips/rocketchip/diplomacy/halfAddr.
	 -freechips/rocketchip/diplomacy/halfAddr#
	 -freechips/rocketchip/diplomacy/halfAddr().
	 -freechips/rocketchip/util/halfAddr.
	 -freechips/rocketchip/util/halfAddr#
	 -freechips/rocketchip/util/halfAddr().
	 -halfAddr.
	 -halfAddr#
	 -halfAddr().
	 -scala/Predef.halfAddr.
	 -scala/Predef.halfAddr#
	 -scala/Predef.halfAddr().
offset: 4470
...
class sdramChipCore(chipId: Int) extends RawModule {
  ...
  withClockAndReset(io.clk.asClock, (!io.cke).asAsyncReset) {
    ...
    if (enable_trace) {
      ...
      when(!io.cs && cmd === CmdRead) {
        printf("chip=%d READ  cs=%d ba=%x row=%x col=%x halfaddr=%x\n",
          chipId.U, io.cs, io.ba, openRow(io.ba), io.a(ColWidth - 1, 0), halfAddr)
      }

      when(!io.cs && cmd === CmdWrite) {
        printf("chip=%d WRITE cs=%d ba=%x row=%x col=%x halfaddr=%x data=%x mask=%x\n",
          chipId.U, io.cs, io.ba, openRow(io.ba), io.a(ColWidth - 1, 0), h@@alfAddr, dqIn, io.dqm)
      }
    }
