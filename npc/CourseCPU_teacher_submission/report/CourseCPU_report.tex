...
\section{RV32I 指令覆盖与支持范围}

作业要求给出 RV32I 指令覆盖率和支持指令表。CourseCPU 的 directed regression 覆盖了课程要求的 25 条核心 RV32I 指令，并额外覆盖了若干完整 RV32I 常用指令，例如无符号比较/分支、半字访存和无符号 load。表 \ref{tab:overview-isa-support} 给出 Overview 级支持范围，附录 \ref{app:rv32i-support} 给出更完整的指令支持表，附录 \ref{app:rv32i-regression-asm} 给出全量指令回归所用汇编源码。

\begin{table}[H]
  \centering
  \caption{CourseCPU RV32I 指令支持与覆盖概览}
  \label{tab:overview-isa-support}
  \begin{tabularx}{\textwidth}{>{\bfseries}lXX}
    \toprule
    类别 & 支持指令 & 覆盖说明 \\
    \midrule
    算术/逻辑 & ADD, SUB, AND, OR, XOR, ADDI, ANDI, ORI, XORI & 课程要求指令，directed regression 覆盖 \\
    移位 & SLL, SRL, SRA, SLLI, SRLI, SRAI & 课程要求指令，directed regression 覆盖 \\
    比较置位 & SLT, SLTI；扩展支持 SLTU, SLTIU & 课程要求有符号比较；扩展覆盖无符号比较 \\
    分支 & BEQ, BNE, BLT, BGE；扩展支持 BLTU, BGEU & 课程要求四类分支；扩展覆盖无符号分支 \\
    跳转 & JAL, JALR & 课程要求指令，覆盖函数调用和返回路径 \\
    访存 & LB, LW, SB, SW；扩展支持 LH, LBU, LHU, SH & 课程要求字节/字访存；扩展覆盖半字和无符号 load \\
    地址生成 & LUI, AUIPC & 课程要求指令，覆盖 PC-relative 和 high immediate 路径 \\
    SYS/CSR & ECALL, MRET, CSR* 可由 \texttt{enableSys} 开启；\texttt{sim ebreak} 可由 \texttt{enableSimEbreak} 开启 & 课程综合/compatible 配置关闭；Verilator difftest 仿真配置开启 CSR/SYS 支持 \\
    \bottomrule
  \end{tabularx}
\end{table}

因此，针对作业要求的 25 条核心指令，CourseCPU 的指令覆盖率为 100\%。更高强度的验证来自两类补充：一是 \texttt{rv32i\_upper\_jump.S}、\texttt{rv32i\_alu\_branch.S} 和 \texttt{rv32i\_load\_store.S} 组成的全量 directed regression；二是由真实 RISC-V GCC 编译的 C benchmark 通过 Verilator + NEMU difftest 逐条提交对拍。

\section{冒险验证程序与波形证明索引}

作业要求提供三个代表性程序证明流水线冒险处理。CourseCPU 将这三类冒险拆成三个短小、可复现的 RTL readmem 程序，并在 Vivado/GTKWave 波形中给出证明。表 \ref{tab:overview-hazard-proof} 给出 Overview 级索引；正文第 \ref{fig:hazard-raw}、\ref{fig:hazard-load-use}、\ref{fig:hazard-flush} 图给出波形截图，附录 \ref{app:hazard-asm} 给出对应汇编源码。

\begin{table}[H]
  \centering
  \caption{CourseCPU 冒险验证程序与波形证明索引}
  \label{tab:overview-hazard-proof}
  \begin{tabularx}{\textwidth}{>{\bfseries}lXXX}
    \toprule
    冒险场景 & 程序 & 波形证明 & 附录源码 \\
    \midrule
    RAW 数据相关 & \texttt{hazard\_raw.S} & \hyperref[fig:hazard-raw]{RAW forwarding 波形} & \hyperref[app:hazard-asm]{Hazard 汇编附录} \\
    Load-use 冒险 & \texttt{hazard\_load\_use.S} & \hyperref[fig:hazard-load-use]{load-use stall 波形} & \hyperref[app:hazard-asm]{Hazard 汇编附录} \\
    控制冒险 flush & \texttt{hazard\_flush.S} & \hyperref[fig:hazard-flush]{branch/JAL flush 波形} & \hyperref[app:hazard-asm]{Hazard 汇编附录} \\
    \bottomrule
  \end{tabularx}
\end{table}
...
SYS/CSR 相关功能由 \texttt{CourseCore} 参数控制。\texttt{CourseTop} 面向 Verilator + NEMU difftest，开启 \texttt{enableSys} 与 \texttt{enableSimEbreak}；\texttt{CourseSynthTop} 与 \texttt{CourseCompatibleTop} 面向综合和 Vivado/readmem RTL testbench，关闭 SYS/CSR 以减少课程配置中不需要的面积和时序开销。

\chapter{Hazard 与 RV32I 回归汇编源码}
\label{app:rv32i-regression-asm}

本附录给出作业要求中可直接复现的汇编源码。前三个程序用于证明 RAW forwarding、load-use stall 和 branch/JAL flush 三类冒险；后三个程序组成 RV32I directed instruction regression，覆盖地址生成、跳转、ALU、分支和 load/store 等指令路径。对应 \texttt{.mem} 文件位于 \texttt{verilog-testbench/resources\_course\_compat/}，可由 \texttt{tb\_CourseCompatibleTop.sv} 通过 \texttt{+IMEM=<path>} 装载。

\section{Hazard 证明程序}
\label{app:hazard-asm}

\lstinputlisting[language={},caption={hazard\_raw.S：连续 ALU RAW forwarding 程序}]{report_assets/hazard_raw.S}

\lstinputlisting[language={},caption={hazard\_load\_use.S：load-use stall 与 forwarding 程序}]{report_assets/hazard_load_use.S}

\lstinputlisting[language={},caption={hazard\_flush.S：branch/JAL redirect 与错误路径 flush 程序}]{report_assets/hazard_flush.S}

\section{RV32I 指令回归程序}

\lstinputlisting[language={},caption={rv32i\_upper\_jump.S：LUI/AUIPC/JAL/JALR 与立即数路径回归}]{report_assets/rv32i_upper_jump.S}

\lstinputlisting[language={},caption={rv32i\_alu\_branch.S：ALU、比较和分支路径回归}]{report_assets/rv32i_alu_branch.S}

\lstinputlisting[language={},caption={rv32i\_load\_store.S：字节、半字和字访存路径回归}]{report_assets/rv32i_load_store.S}
