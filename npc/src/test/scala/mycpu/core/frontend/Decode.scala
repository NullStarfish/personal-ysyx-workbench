...
class DecodeSim extends AnyFlatSpec {
  ...
  it should "decode ADDI as rs1 plus sign-extended immediate" in {
    simulate(new Decode) { c =>
      ...
      c.io.out.bits.imm.expect("hfffffffc".U)
  ...
  it should "decode SW with both source registers and store metadata" in {
    simulate(new Decode) { c =>
      ...
      c.io.out.bits.imm.expect(12.U)
      c.io.out.bits.exec.aluOp.expect(ALUOp.ADD)
      c.io.out.bits.exec.aluSrcA.expect(ALUSrcA.Rs1)
      c.io.out.bits.exec.aluSrcB.expect(ALUSrcB.Imm)
      c.io.out.bits.wb.wen.expect(false.B)
      c.io.out.bits.mem.valid.expect(true.B)
      c.io.out.bits.mem.write.expect(true.B)
      c.io.out.bits.mem.unsigned.expect(false.B)
  ...
  it should "decode BEQ as a branch with register operands and B immediate" in {
    simulate(new Decode) { c =>
      ...
      c.io.out.bits.imm.expect(16.U)
      c.io.out.bits.exec.branchType.expect(BranchType.Eq)
      c.io.out.bits.exec.isJump.expect(false.B)
      c.io.out.bits.wb.wen.expect(false.B)
      c.io.out.bits.mem.valid.expect(false.B)
  ...
  it should "decode JAL as PC plus offset and write pc plus 4" in {
    simulate(new Decode) { c =>
      ...
      c.io.out.bits.rs2.valid.expect(false.B)
      c.io.out.bits.imm.expect(32.U)
      c.io.out.bits.exec.aluOp.expect(ALUOp.ADD)
      c.io.out.bits.exec.aluSrcA.expect(ALUSrcA.Pc)
      c.io.out.bits.exec.aluSrcB.expect(ALUSrcB.Imm)
      c.io.out.bits.exec.wbSel.expect(WBSel.PcPlus4)
      c.io.out.bits.exec.isJump.expect(true.B)
      c.io.out.bits.exec.isJalr.expect(false.B)
      c.io.out.bits.wb.wen.expect(true.B)
  ...
  it should "decode CSRRSI using zimm as rs1 data and CSR writeback" in {
    simulate(new Decode) { c =>
      ...
      c.io.out.bits.wb.wen.expect(true.B)
      c.io.out.bits.wb.rd.expect(10.U)
      c.io.out.bits.sys.csrOp.get.expect(CSROp.S)
      c.io.out.bits.sys.csrAddr.get.expect("h305".U)
      c.io.out.bits.sys.isEcall.get.expect(false.B)
      c.io.out.bits.sys.isMret.get.expect(false.B)
      c.io.out.bits.sys.isEbreak.get.expect(false.B)
