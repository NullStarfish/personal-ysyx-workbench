...
ifeq ($(strip $(CONFIG_CACHESIM_DIFFTEST)),y)
CONFIG_CPPFLAGS += -DCONFIG_CACHESIM_DIFFTEST
endif
...
BUILD_MODE_TEXT = SIM_TARGET=$(SIM_TARGET) BOARD=$(BOARD) VERILATOR=$(shell command -v $(VERILATOR)) VERILATOR_ROOT=$(VERILATOR_ROOT) CONFIG_TRACE=$(CONFIG_TRACE) CONFIG_ITRACE=$(CONFIG_ITRACE) CONFIG_FTRACE=$(CONFIG_FTRACE) CONFIG_MTRACE=$(CONFIG_MTRACE) CONFIG_DTRACE=$(CONFIG_DTRACE) CONFIG_CACHESIM_DIFFTEST=$(CONFIG_CACHESIM_DIFFTEST) CONFIG_DIFFTEST=$(CONFIG_DIFFTEST) CONFIG_WATCHPOINT=$(CONFIG_WATCHPOINT)
